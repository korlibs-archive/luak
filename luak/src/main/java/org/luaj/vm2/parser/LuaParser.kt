...
class LuaParser : LuaParserConstants {
    ...
    @JvmOverloads
    constructor(stream: java.io.InputStream, encoding: String? = null) {
        try {
    ...
    /** Reinitialise.  */
    @JvmOverloads
    fun ReInit(stream: java.io.InputStream, encoding: String? = null) {
        try {
    ...
    /** Reinitialise.  */
    fun ReInit(stream: java.io.Reader) {
    ...
    private fun jj_consume_token(kind: Int): Token {
        ...
        if ((run { oldToken = token; oldToken })?.next != null)
            token = token!!.next
        else {
            token!!.next = token_source.nextToken
            token = token!!.next
    ...
    private fun jj_ntk(): Int {
        if ((run { jj_nt = token!!.next; jj_nt }) == null)
            jj_ntk = (run { token!!.next = token_source.nextToken; token_source.nextToken }).kind
        else
            jj_ntk = jj_nt!!.kind
        return jj_ntk
    }
    ...
    fun generateParseException(): ParseException {
        ...
        val exptokseq = Array<IntArray>(jj_expentries.size) { jj_expentries!!.get(it)!! }
        return ParseException(token!!, exptokseq, LuaParserConstants.tokenImage)
    }
