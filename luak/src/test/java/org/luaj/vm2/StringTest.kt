...
class StringTest : TestCase() {
    ...
    fun testToInputStream() {
        ...
        TestCase.assertEquals('H', `is`.read().toChar())
        TestCase.assertEquals('e', `is`.read().toChar())
        TestCase.assertEquals(2, `is`.skip(2))
        TestCase.assertEquals('o', `is`.read().toChar())
        ...
        TestCase.assertEquals('e', `is`.read().toChar())
        TestCase.assertEquals('l', `is`.read().toChar())
        TestCase.assertEquals('l', `is`.read().toChar())
        TestCase.assertEquals(-1, `is`.read())
    ...
    fun testSpotCheckUtf8() {
        ...
        TestCase.assertEquals(160, d[0].toInt())
        TestCase.assertEquals(161, d[1].toInt())
        TestCase.assertEquals(162, d[2].toInt())
        TestCase.assertEquals(163, d[3].toInt())
        TestCase.assertEquals(164, d[4].toInt())
    ...
    fun testRecentStringsCacheDifferentHashcodes() {
        ...
        TestCase.assertTrue(abc1.hashCode() % mod != xyz1.hashCode() % mod)
        TestCase.assertSame(abc1, abc2)
        TestCase.assertSame(xyz1, xyz2)
    }
    ...
    fun testRecentStringsCacheHashCollisionCacheHit() {
        ...
        TestCase.assertNotSame(abc1, lyz1)
        TestCase.assertFalse(abc1 == lyz1)
        TestCase.assertSame(abc1, abc2)
        TestCase.assertSame(lyz1, lyz2)
    }
    ...
    fun testRecentStringsCacheHashCollisionCacheMiss() {
        ...
        TestCase.assertNotSame(abc1, lyz1)
        TestCase.assertFalse(abc1 == lyz1)
        TestCase.assertNotSame(abc1, abc2)
        TestCase.assertNotSame(lyz1, lyz2)
    }

    fun testRecentStringsLongStrings() {
        val abc = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".toByteArray()
        TestCase.assertTrue(abc.size > LuaString.RECENT_STRINGS_MAX_LENGTH)
        val abc1 = LuaString.valueOf(abc)
        val abc2 = LuaString.valueOf(abc)
        TestCase.assertNotSame(abc1, abc2)
    }
    ...
    fun testRecentStringsUsingJavaStrings() {
        ...
        val mod = LuaString.RECENT_STRINGS_CACHE_SIZE
        TestCase.assertEquals(abc1.hashCode() % mod, lyz1.hashCode() % mod)
        TestCase.assertFalse(abc1.hashCode() % mod == xyz1.hashCode() % mod)
        TestCase.assertSame(abc1, abc2)
        TestCase.assertSame(lyz1, lyz2)
        TestCase.assertSame(xyz1, xyz2)
        ...
        TestCase.assertNotSame(abc3, abc4)  // because of hash collision
        TestCase.assertNotSame(lyz3, lyz4)  // because of hash collision
        TestCase.assertSame(xyz3, xyz4)  // because hashes do not collide
    }
    ...
    fun testLongSubstringGetsOldBacking() {
        ...
        TestCase.assertSame(src.m_bytes, sub1.m_bytes)
    ...
    fun testShortSubstringGetsNewBacking() {
        ...
        TestCase.assertSame(sub1, sub2)
        TestCase.assertFalse(src.m_bytes == sub1.m_bytes)
    }
    ...
    fun testShortSubstringOfVeryLongStringGetsNewBacking() {
        ...
        TestCase.assertFalse(sub1 === sub2)
        TestCase.assertFalse(src.m_bytes == sub1.m_bytes)
    }
    ...
    fun testIndexOfByteInSubstring() {
        ...
        TestCase.assertEquals(6, str.indexOf(':'.toByte(), 0))
        TestCase.assertEquals(6, str.indexOf(':'.toByte(), 2))
        TestCase.assertEquals(6, str.indexOf(':'.toByte(), 6))
        TestCase.assertEquals(-1, str.indexOf(':'.toByte(), 7))
        TestCase.assertEquals(-1, str.indexOf(':'.toByte(), 9))
        TestCase.assertEquals(9, str.indexOf('i'.toByte(), 0))
        TestCase.assertEquals(9, str.indexOf('i'.toByte(), 2))
        TestCase.assertEquals(9, str.indexOf('i'.toByte(), 9))
        TestCase.assertEquals(-1, str.indexOf('z'.toByte(), 0))
        TestCase.assertEquals(-1, str.indexOf('z'.toByte(), 2))
        TestCase.assertEquals(-1, str.indexOf('z'.toByte(), 9))

        TestCase.assertEquals(4, sub.indexOf(':'.toByte(), 0))
        TestCase.assertEquals(4, sub.indexOf(':'.toByte(), 2))
        TestCase.assertEquals(4, sub.indexOf(':'.toByte(), 4))
        TestCase.assertEquals(-1, sub.indexOf(':'.toByte(), 5))
        TestCase.assertEquals(-1, sub.indexOf(':'.toByte(), 7))
        TestCase.assertEquals(7, sub.indexOf('i'.toByte(), 0))
        TestCase.assertEquals(7, sub.indexOf('i'.toByte(), 2))
        TestCase.assertEquals(7, sub.indexOf('i'.toByte(), 7))
        TestCase.assertEquals(-1, sub.indexOf('z'.toByte(), 0))
        TestCase.assertEquals(-1, sub.indexOf('z'.toByte(), 2))
        TestCase.assertEquals(-1, sub.indexOf('z'.toByte(), 7))
    }
    ...
    fun testMatchShortPatterns() {
        ...
        TestCase.assertEquals(nil, vvv.invokemethod("match", args))
        TestCase.assertEquals(nil, a.invokemethod("match", args))
        TestCase.assertEquals(nil, ax.invokemethod("match", args))
        TestCase.assertEquals(nil, axb.invokemethod("match", args))
        TestCase.assertEquals(xby, axby.invokemethod("match", args))
        TestCase.assertEquals(xby, xbya.invokemethod("match", args))
        TestCase.assertEquals(nil, bya.invokemethod("match", args))
        TestCase.assertEquals(xby, xby.invokemethod("match", args))
        TestCase.assertEquals(xby, axbya.invokemethod("match", args))
        TestCase.assertEquals(xby, axbya.substring(0, 4).invokemethod("match", args))
        TestCase.assertEquals(nil, axbya.substring(0, 3).invokemethod("match", args))
        TestCase.assertEquals(xby, axbya.substring(1, 5).invokemethod("match", args))
        TestCase.assertEquals(nil, axbya.substring(2, 5).invokemethod("match", args))
    }
}
